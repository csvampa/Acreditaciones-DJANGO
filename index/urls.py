from django.urls import path
from .views import Personas, Eventos
from . import views

urlpatterns = [
    # path('index/', views.db_evento, name='index'),
    path('accounts/profile/<int:evento_id>/<str:evento_nombre>/', Personas.as_view(), name='db_evento'),
    path('accounts/profile/<int:evento_id>/<str:evento_nombre>/cargaIndividual', Personas.as_view(), name='cargaIndividual'),
    path('accounts/profile/<int:evento_id>/<str:evento_nombre>/cargaMasiva', Personas.as_view(), name='cargaMasiva'),
    path('index/error.html', views.error_view, name='error'),
    path('eventos/', Eventos.as_view(), name='eventos'),
    # path('actualizar_asistencia/', views.actualizar_asistencia, name='actualizar_asistencia'),
    ]